...
function formatTradeForAnalysis(trade) {
  ...
  return {
    ...
    emotionDetected: trade.emotionAnalysis?.detected || null,
    tags: trade.tags || [],
    // Discipline tracking - important for pattern detection
    discipline: {
      ...
      slMovementReason: trade.stopLossMovementReason || null,
      earlyExit: trade.earlyExit?.exitedBeforeTarget || false,
      earlyExitReason: trade.earlyExit?.exitReason || null,
...
async function generateWeeklyInsights(userId, startDate, endDate) {
  try {
    ...
    // Discipline analysis - CRITICAL for behavioral insights
    const closedTrades = trades.filter(t => t.result && t.result !== 'open');
    const disciplineStats = {
      tradesWithStopLoss: closedTrades.filter(t => t.stopLoss).length,
      tradesMovedSLDown: closedTrades.filter(t => t.movedStopLossDown).length,
      lossesFromMovedSL: closedTrades.filter(t => t.movedStopLossDown && t.result === 'loss').length,
      earlyExits: closedTrades.filter(t => t.earlyExit?.exitedBeforeTarget).length,
      earlyExitReasons: closedTrades
        .filter(t => t.earlyExit?.exitReason)
        .reduce((acc, t) => {
          const reason = t.earlyExit.exitReason;
          acc[reason] = (acc[reason] || 0) + 1;
          return acc;
        }, {}),
      slMovementReasons: closedTrades
    ...
    const analysisPayload = {
      ...
      // Discipline tracking - KEY for identifying indiscipline patterns
      disciplineStats: {
        ...disciplineStats,
        slUsageRate: closedTrades.length > 0 
          ? Math.round((disciplineStats.tradesWithStopLoss / closedTrades.length) * 100)
          : 0,
        slViolationRate: disciplineStats.tradesWithStopLoss > 0
          ? Math.round((disciplineStats.tradesMovedSLDown / disciplineStats.tradesWithStopLoss) * 100)
          : 0,
        earlyExitRate: closedTrades.length > 0
          ? Math.round((disciplineStats.earlyExits / closedTrades.length) * 100)
          : 0
      },
